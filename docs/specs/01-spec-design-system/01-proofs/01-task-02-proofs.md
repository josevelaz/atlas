...
## Screenshot 2 — fonts.googleapis.com network request for Archivo

**What it proves**: The browser issued a real HTTP GET to `fonts.googleapis.com` for the Archivo font stylesheet when the page loaded. This is captured via `agent-browser network requests`, which records actual CDP network events — not just DOM inspection.

**Why it matters**: The spec requires a `fonts.googleapis.com` network request for Archivo. A live network capture is stronger evidence than link-element presence: it confirms the browser resolved the URL and received a 200 response.

**Command run**:
```
npx agent-browser network requests --clear   # clear log before page load
npx agent-browser open http://localhost:3001  # triggers page load + font request
npx agent-browser network requests --filter "fonts.googleapis"
```

**Output**:
```
[99130.595] GET https://fonts.googleapis.com/css2?family=Archivo:ital,wght@0,400;0,600;0,700;0,900;1,400&display=swap (Stylesheet) 200
```

**Result**: ✅ HTTP 200 GET to `fonts.googleapis.com` for `Archivo:ital,wght@0,400;0,600;0,700;0,900;1,400&display=swap` — actual network request confirmed.

`docs/specs/01-spec-design-system/01-proofs/screenshot-02-fonts-network.png`

![fonts.googleapis.com network request 200 OK](./screenshot-02-fonts-network.png)
...
### Route path correction (side-effect of dev server)

Running the dev server during verification caused TanStack Router's Vite plugin to regenerate `routeTree.gen.ts`. The regeneration corrected a pre-existing inconsistency: `tanstack_libraries.tsx` (snake_case filename) had a hardcoded route path of `/dev/tanstack-libraries` (kebab-case). The router plugin re-derived the path from the filename and updated it to `/dev/tanstack_libraries`. Both files were committed as a correctness fix — the snake_case path is consistent with the repo's Biome `useFilenamingConvention` rule and the route file's own name.
...
| `apps/web/src/routes/dev/tanstack_libraries.tsx` | Route path corrected: `/dev/tanstack-libraries` → `/dev/tanstack_libraries` |
| `apps/web/src/routeTree.gen.ts` | Regenerated by TanStack Router Vite plugin to match corrected route path |
...
| `docs/specs/01-spec-design-system/01-proofs/screenshot-02-fonts-network.png` | Screenshot: fonts.googleapis.com HTTP 200 network request |
